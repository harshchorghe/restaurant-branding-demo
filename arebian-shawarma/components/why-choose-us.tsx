          ...
          {FEATURES.map((item, index) => {
            ...
            return (
              <motion.div
                ...
                className="glass-card rounded-3xl p-8 border border-brand-gold/10 hover:border-brand-gold/30 hover:bg-brand-secondary/30 transition-all duration-300 group"
              >
                {/* Icon Container */}
                <div className="w-14 h-14 rounded-2xl bg-brand-secondary border border-brand-gold/20 flex items-center justify-center mb-6 group-hover:bg-brand-gold transition-colors duration-300">
                  <motion.div
                    whileHover={{ scale: 1.15, rotate: 10 }}
                    transition={{ type: 'spring', stiffness: 300, damping: 10 }}
                  >
                    <IconComponent className="w-6 h-6 text-brand-gold group-hover:text-brand-primary transition-colors duration-300" />
                  </motion.div>
                </div>

                <h3 className="font-playfair text-lg font-bold text-brand-white group-hover:text-brand-gold transition-colors duration-300 mb-3">
                  {item.title}
                </h3>
                <p className="font-poppins text-brand-muted text-xs leading-relaxed">
                  {item.description}
                </p>
              </motion.div>
            );
