...
export default function Gallery() {
  ...
  return (
    <section id="gallery" className="relative py-8 md:py-24 bg-bg-secondary px-6 md:px-12 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto z-10 relative">
        ...
        <div className="relative w-full overflow-hidden py-4 -mx-6 md:-mx-12 px-6 md:px-12">
          ...
          <div className="absolute top-0 bottom-0 left-0 w-20 sm:w-32 bg-gradient-to-r from-bg-secondary via-bg-secondary/50 to-transparent z-25 pointer-events-none" />
          <div className="absolute top-0 bottom-0 right-0 w-20 sm:w-32 bg-gradient-to-l from-bg-secondary via-bg-secondary/50 to-transparent z-25 pointer-events-none" />
          ...
          <div className="animate-marquee">
            {[...IMAGES, ...IMAGES, ...IMAGES].map((img, idx) => (
              <div
                ...
                className="relative w-64 h-48 sm:w-80 sm:h-56 rounded-3xl border border-border cursor-pointer group shadow-lg shrink-0 mx-4 overflow-hidden"
              >
                {/* Image element */}
                <Image
                  src={img.src}
                ...
                <div className="absolute inset-0 bg-bg-primary/60 backdrop-blur-xs opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10 flex flex-col items-center justify-center p-6 text-center">
                  <div className="w-10 h-10 rounded-full bg-brass-primary/20 border border-brass-primary flex items-center justify-center mb-3">
                    <Search className="w-4 h-4 text-brass-primary" />
                  </div>
                  <h3 className="font-playfair text-sm sm:text-base font-bold text-text-primary uppercase tracking-wider">
                    {img.title}
                  </h3>
                  <span className="font-montserrat text-[8px] text-text-secondary uppercase tracking-[0.25em] mt-1 block">
                    Click to Expand
                  </span>
                </div>
              </div>
      ...
      <AnimatePresence>
        {selectedImage && (
          <motion.div
            ...
            className="fixed inset-0 z-[9999] bg-bg-primary/95 backdrop-blur-md flex items-center justify-center p-4 md:p-10"
          >
            {/* Close Button */}
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-6 right-6 z-50 w-12 h-12 rounded-full bg-bg-secondary/80 border border-border flex items-center justify-center text-text-primary hover:text-brass-bright transition-colors active:scale-95"
            ...
            <motion.div
              ...
              className="relative w-full max-w-5xl h-[75vh] md:h-[80vh] rounded-3xl overflow-hidden border border-border-strong shadow-2xl"
            >
              <Image
                src={selectedImage}
